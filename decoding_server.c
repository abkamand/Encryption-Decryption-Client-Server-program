...
//sendall() from beej's, receiveall() adapted from this                  
  ...
  while(1){
    ...
    */                     
    ...
    if (!fork())
    {
      ...
      if (strcmp("./dec_client", server_name) != 0)
      {
      ...
      //decode plaintext and send back
      char decoded_buffer[SIZE];
      int temp;
      for (int i = 0; i < input_size; i++)
      {
          ...
          //subtract ASCII values
          temp = (plaintext[i] - key[i]);
          if (temp < 0)
          {
              temp = temp + 27;
          }
          //store
          decoded_buffer[i] = temp + 65;
          //convert '[' back to ' ' for proper output
          if (decoded_buffer[i] == '[')
          {
            decoded_buffer[i] = ' ';
          }
      ...
      if (sendall(connectionSocket, decoded_buffer, &input_size) == -1)
      {
